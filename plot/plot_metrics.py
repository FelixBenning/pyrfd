...
def plot_summary(problem):
    ...
    for (idx, item) in enumerate(metrics):
        plot_validation_loss(axs[0], item, idx=idx)
        plot_initial_learning_rate(axs[1], item, idx=idx)
...
def plot_performance(problem):
    ...
    plot_final_loss_over_lr(axs[1, 1], {"name": "Adam", "metrics": adam_joined}, idx=3)

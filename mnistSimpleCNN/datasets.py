...
class MnistDataset(torch.utils.data.Dataset):
    def __init__(self, training=True, transform=None):
        if training == True:
            f = open(DIR.joinpath("data/MNIST/raw/train-images-idx3-ubyte"), "rb")
            xs = np.array(np.frombuffer(f.read(), np.uint8, offset=16))
            f.close()
            f = open(DIR.joinpath("data/MNIST/raw/train-labels-idx1-ubyte"), "rb")
            ys = np.array(np.frombuffer(f.read(), np.uint8, offset=8))
            f.close()
        else:
            f = open(DIR.joinpath("data/MNIST/raw/t10k-images-idx3-ubyte"), "rb")
            xs = np.array(np.frombuffer(f.read(), np.uint8, offset=16))
            f.close()
            f = open(DIR.joinpath("data/MNIST/raw/t10k-labels-idx1-ubyte"), "rb")
            ys = np.array(np.frombuffer(f.read(), np.uint8, offset=8))
            f.close()
